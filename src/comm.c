...
#define FNDELAY O_NDELAY
    ...
    *dnew		= d_zero;
	...
	sprintf( buf, "¥Ø«e¦³[1;33m%3d[m¦ìª±®a¦b½u¤W, ¦³[1;32m%3d[m¦ì¬O¤j¯«, ¦³[1;31m%3d[m¦ì¬O¹ê²ß¯«\n\r[1;33m¨t²Î¤½§i: %s[m\n\r", players_count, directs_count, immortals_count, sys_ann );
	write_to_buffer( dnew, buf, 0 );
	write_to_buffer( dnew, "½Ð¿é¤J§Aªº¦W¦r: ", 0 );
    }
...
void close_socket( DESCRIPTOR_DATA *dclose )
{
	...
	{
	    act( "$c¥¢¥h¤F$sªº³s½u¡C", ch, NULL, NULL, TO_ROOM );
	    ch->desc = NULL;
	}
	else
	{
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	{
	    write_to_buffer( d, "¤£¦Xªkªº¦W¦r¡A´«¤@­Ó¸Õ¸Õ¡C\n\r½Ð¿é¤J§Aªº¦W¦r: ", 0 );
	    return;
	}
	...
	if ( IS_SET( ch->act, PLR_DENY ) )
	{
	    ...
	    log_string( log_buf );
	    write_to_buffer( d, "[1;33m§A³Q¸T¤î³s½u¶i¤J¥»MUD¡C[m\n\r", 0 );
	    close_socket( d );
	    return;
	}
	...
	else
	{
	    ...
	    {
		write_to_buffer( d, "[1;31m²{¦b³o­ÓMUD¥u¦³[1;33m¤j¯«[1;31m¤~¥i¥H¶i¤J¡C[m\n\r", 0 );
		close_socket( d );
		return;
	    }
	    if ( ch->level <= numlock
	    ...
	    {
		write_to_buffer( d,
			"³o­Ó¹CÀ¸²{¦b¤£¶}©ñµ¹±z³o­Óµ¥¯Åªºª±®a¨Ï¥Î¡C\n\r\n\r",
				0 );
		if ( ch->level == 0 )
		{
		    write_to_buffer( d,
			"·sªºª±®a²{¦b¬O¥H Email »{ÃÒ¤è¦¡¨Óµù¥U¤@­Ó¨¤¦â¡C\n\r\n\r",
				    0 );
		    write_to_buffer( d,
			"½Ðµo¤@«Ê email ¨ì mudadm@mud.ck.tp.edu.tw ¨Óµù¥U§Aªº¨¤¦â¡C\n\r", 0 );
		    write_to_buffer( d,
			"¤@­Ó email ¦ì§}¤@­Ó¨¤¦â, ½Ð¶ñ¤J±zªº­Ó¤H¸ê®Æ¤@¨Ö±H¨Ó, ¤j¯«±N·|¬°±z³B²z¡C\n\r\n\r", 0 );
		}
		close_socket( d ) ;
		return;
	    }
	...
	if ( fOld )
	{
	    ...
	    {
	        sprintf( buf, "§A¤W¦¸¬O±q %s ³s½u¶i¨Óªº¡C\n\r", d->character->pcdata->lastFrom	         );
	        write_to_buffer( d, buf, 0 );
	    }
	    write_to_buffer( d, "[m½Ð¿é¤J±zªº±K½X: ", 0 );
	...
	{
	    /* New player */
	    sprintf( buf, "£°..§Ú¨S¬Ý¿ù§a, §Aªº¦W¦r¬O[1;34m %s [m(Y/N)? ", argument );
	    write_to_buffer( d, buf, 0 );
	...
	{
	    write_to_buffer( d, "[1;31m±K½X¿ù»~¡C[m\n\r", 0 );
	    write_to_buffer( d, "[0;32m¦p¦³¥ô¦ó°ÝÃD, Åwªï¨Ó«H mudadm@mud.ck.tp.edu.tw ¡C[m\n\r", 0 );
	    close_socket( d );
	    return;
	}
	...
	{
	case 'y': case 'Y':
	    sprintf( buf, "£¬..§Ú¨S¨£¹L§A, ¬O[0;33m·sªºª±®a[m°Ú!!\n\r½Ðµ¹§Ú[0;32m %s [mªº±K½X:%s",
	...
	case 'n': case 'N':
	    write_to_buffer( d, "¦n§a, ¤£µM¬O¤°»ò©O? ", 0 );
	...
	default:
	    write_to_buffer( d, "½Ð¿é¤J[1;34m Yes [m©Î[1;35m No [m? ", 0 );
	    break;
	}
	break;
	...
	for ( p = pwdnew; *p != '\0'; p++ )
	{
	    ...
	    {
		write_to_buffer( d,
		    "¤£±µ¨ü§A·sªº±K½X, ¦A¸Õ¸Õ¨ä¥Lªº¬Ý¬Ý¡C\n\r±zªº±K½X: ",
		    0 );
		return;
	    }
	}
	...
	write_to_buffer( d, "½Ð¦A¿é¤J¤@¦¸±K½X: ", 0 );
	...
	write_to_buffer( d, echo_on_str, 0 );
	write_to_buffer( d, "\n\r½ÐÀ£¤U [1;33mENTER[m ÁäÄ~Äò:\n\r", 0 );
	write_to_buffer( d, "\n\r½Ð°Ý§Aªº©Ê§O¬O( [1;37mM[male(¡ñ)/[1;37mF[memale(¡ð)/[1;37mN[meutral(¡ò) )? ", 0 );
...
/*    case CON_DISPLAY_RACE:
	...
	write_to_buffer( d, "§A½T©w§A­n³o­ÓºØ±Ú¶Ü? ", 0 );
	...
	default:
	    write_to_buffer( d, "¨º¤£¬O¤@­Ó©Ê§O¡C\n\r½Ð°Ý§Aªº©Ê§O? ", 0 );
	    return;
	}

	d->connected = CON_READ_MOTD;
	write_to_buffer( d, "\n\r½ÐÀ£¤U [1;36mENTER[m Ä~Äò:\n\r", 0 );
	break;
	...
	send_to_char(
    "[H[J\n\r§A©¿µMÄ±±o[1;37mÀY¸£[m°{¤F¤@¤U¡C¦b¤@Àþ¶¡«á, §A¸£¸Ìªº[0;34mµøÄ±°Ï¸ê®Æ[m¤w³Q§ó·s¡C\n\r",
	    ch );
	send_to_char(
    "¬M¤J§A²´«eªº, ¬O¤@­Ó[1;31m¤£¥i«äÄ³ªº¥@¬É[m¡C¹Ú·Q¡B§Æ±æ¥Rº¡¦b³o¸Ì¡C\n\r", ch );
        send_to_char(
    "³o¬O¥Ñ[1;33m·s¤@¥Nªº¤Ñ¯«[m, ©Ò³Ð³y¥X¨Óªº¥@¬É¡Ð[1;32mÁ¼ÇU»â°ì¡Ð¹FÄõ¶ð[m\n\r", ch );
        send_to_char(
    "-----------------------------------------------------------------------------\n\r\n\r\n\r", ch );
	...
	if ( ch->level == 0 )
	{
	    ...
	    ch->pcdata->prompt = str_dup( "<¥Í[0;35m%h[mºë[1;34m%m[mÅé[0;32m%v[m°«[0;33m%A[m> " );
	...
	if ( !IS_SET( ch->act, PLR_WIZINVIS )
	    ...
	    act( "[1;35m$c[1;35m¶i¤J¤F³o­Ó»â°ì¡C[m", ch, NULL, NULL, TO_ROOM );
	...
	if ( notes == 1 )
	    send_to_char( "\n\r[1;32m§A¦³¤@«Ê·s¨ìªº¶l¥ó¡C[m\n\r", ch );
	else
	    if ( notes > 1 )
	    {
		sprintf( buf, "\n\r[1;32m§A¦³%3d«Ê·s¨ìªº¶l¥ó¡C[m\n\r",
			notes );
		send_to_char( buf, ch );
	    }
...
bool check_playing( DESCRIPTOR_DATA *d, char *name )
{
	...
	{
	    write_to_buffer( d, "¤w¸g¦b¹CÀ¸¤¤¤F¡C\n\r½Ð¦A¿é¤J±zªº¦W¦r: ", 0 );
	    d->connected = CON_GET_NAME;
	    if ( d->character )
	    {
		free_char( d->character );
