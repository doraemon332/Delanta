 ...
 *								           *
 *  DoraemonMud 0.1b1 improvements copyright (C) 1997 by Doraemon.	   *
 *                                                                         *
 *  In order to use any part of this Envy Diku Mud, you must comply with   *
 *  the original Diku license in 'license.doc', the Merc license in        *
 *  'license.txt', as well as the Envy license in 'license.nvy' and the    *
 *  Doraemon license in 'license.dor'.				           *
...
void	load_cmobiles    args( ( FILE *fp ) );
void	load_cobjects    args( ( FILE *fp ) );
...
void boot_db( void )
{
	...
	long lhour, lday, lmonth;
        log_string( "Setting up time and weather.." );	
	...
	if ( !( version = fopen( OS_VERSION, "r" ) ) )
	{
	    perror( OS_VERSION );
	    strcpy( os_version, "Unknown Operating System\n\r\n\r" );
	}
	else 
	{
	    int stat;
	    
	    log_string( "Setting up Operating System version.." );
	    
	    os_version = fread_string( version, &stat );
	    fclose( version );
	}
	
	if ( !( announce = fopen( SYS_ANN, "r" ) ) )
	{
	    perror( SYS_ANN );
	    strcpy( sys_ann, "¨S¦³¨t²Î¤½§i¡C\n\r" );
	}
	else
	{
	    int stat;
	    log_string( "Setting up system announce .." );
	    sys_ann = fread_string( announce, &stat );
	    fclose( announce );
	}
	...
	for ( ; ; )
	{
		...
		word = fread_word( fpArea, &stat );
		    ...
		    load_mobiles ( fpArea );
	        else if ( !str_cmp( word, "CMOBILES" ) )
	            load_cmobiles( fpArea );
	        else if ( !str_cmp( word, "MOBPROGS" ) )
	            load_mobprogs( fpArea );
		else if ( !str_cmp( word, "OBJECTS"  ) )
		    load_objects ( fpArea );
		else if ( !str_cmp( word, "COBJECTS"  ) )
		    load_cobjects( fpArea );
	...
	fix_exits( );
	fBootDb  = FALSE;
	daPrompt = str_dup( "<¥Í[0;35m%h[mºë[1;34m%m[mÅé[0;32m%v[m°«[0;33m%A[m> " );
	boot_done( );
	log_string( "Reupdating Areas.." );
	area_update( );
...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->cname		= str_dup( pMobIndex->short_descr );
 ...
 * Snarf a mob section.
 */
void load_cmobiles( FILE *fp )
{
    MOB_INDEX_DATA *pMobIndex;

    if ( !area_last )
    {
    	bug( "Load_cmobiles: no #AREA seen yet.", 0 );
    	exit( 1 );
    }
...
void load_objects( FILE *fp )
{
	...
	pObjIndex->cname		= str_dup( pObjIndex->short_descr );
...
void load_social (FILE *fp, struct social_type *social)
{
	...
	social->name =          fread_string (fp, &stat);
	social->char_no_arg =   fread_string (fp, &stat);
	social->others_no_arg = fread_string (fp, &stat);
	social->char_found =    fread_string (fp, &stat);
	social->others_found =  fread_string (fp, &stat);
	social->vict_found =    fread_string (fp, &stat);
	social->char_auto =     fread_string (fp, &stat);
	social->others_auto =   fread_string (fp, &stat);
}
...
CHAR_DATA *new_character( bool player )
{
	...
	*ch->pcdata = pcdata_zero;
    ...
    ch->skill_level = alloc_perm( sizeof( int ) * MaxSkill );
    for ( i = 0; i < MaxSkill; i++ )
        ch->skill_level[i] = 1000;    
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->cname		= str_dup( pMobIndex->cname       );
...
void free_char( CHAR_DATA *ch )
{
	...
	free_string( ch->pcdata->lastFrom	);
	free_string( ch->pcdata->lastName	);
	
	ch->pcdata->next = pcdata_free;
	ch->pcdata->angry_point = 50;
	ch->pcdata->max_ap	= 100;
...
char *fread_word( FILE *fp, int *status )
{
	...
	*pword = getc( fp );
    ...
    sprintf( buf, "The word is %s", word );
    bug( buf, 0 );    
    *status = 1;
    return NULL;
}
...
void mprog_read_programs( FILE *fp, MOB_INDEX_DATA *pMobIndex)
{
  ...
  log_string( "Read Program" );
  return;

}

int _sort_descriptor_by_level( d1, d2 )
DESCRIPTOR_DATA **d1, **d2;
{
    return ( (*d1)->character->level < (*d2)->character->level);
}

int _sort_descriptor_by_name( d1, d2 )
DESCRIPTOR_DATA **d1, **d2;
{
    return str_cmp( (*d1)->character->name, (*d2)->character->name );
}

void sort_descriptor( int sorting )
{
    DESCRIPTOR_DATA *d;
    DESCRIPTOR_DATA **d_array;
    int count, i;

    for ( count = 0, d = descriptor_list; d; d = d->next, count++ );
    d_array = (DESCRIPTOR_DATA **)alloc_mem( sizeof(DESCRIPTOR_DATA *) * count );
    
    for ( i = 0, d = descriptor_list; d; d = d->next, i++ )
        d_array[i] = d;
        
    qsort( d_array, count, sizeof(d_array[0]), sorting );
    
    for ( i = 0; i < count-1; i++ )
        d_array[i]->next = d_array[i+1];
        
    d_array[count-1]->next = NULL;
    descriptor_list = d_array[0];
    free_mem( d_array, sizeof(DESCRIPTOR_DATA *) * count );
    return;
}
