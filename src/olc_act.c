...
bool oedit_cname( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax:  cname [string]\n\r", ch );
	return FALSE;
    }
...
bool medit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "AC:          [%d]\n\rHitroll:     [%d]\n\rDamroll:     [%d]\n\r"
                   , pMob->ac, pMob->hitroll, pMob->damplus );
    send_to_char( buf, ch );
...
bool medit_gold( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ))
    {
        send_to_char( "粂猭 gold [计]\n\r", ch );
        return FALSE;
    }
    
    pMob->gold = atoi( argument );
    
    send_to_char( "砞﹚MOB窥\n\r", ch );
    return TRUE;
}
...
bool medit_ac( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ))
    {
        send_to_char( "粂猭 ac [计]\n\r", ch );
        return FALSE;
    }    

    pMob->ac = atoi( argument );
    
    send_to_char( "砞﹚MOBň縨计\n\r", ch );
    return TRUE;    
}
...
bool medit_damroll( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ))
    {
        send_to_char( "粂猭 damroll [计]\n\r", ch );
        return FALSE;
    }    

    pMob->damplus = atoi( argument );
    
    send_to_char( "砞﹚MOB端甡\n\r", ch );
    return TRUE;    
}
...
bool medit_hitroll( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ))
    {
        send_to_char( "粂猭 hitroll [计]\n\r", ch );
        return FALSE;
    }    

    pMob->hitroll = atoi( argument );
    
    send_to_char( "砞﹚MOB㏑い瞯\n\r", ch );
    return TRUE;    
}
