...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    sprintf( buf2, "$n口中喃喃唸著咒語,「%s」", buf );
    sprintf( buf,  "$n口中喃喃唸著咒語,「%s」", skill_table[sn].name );
...
void spell_smash( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    act( "忽然有一道閃電打在$c的身上。", victim, NULL, NULL, TO_ROOM );
    damage( ch, victim, dam, sn, WEAR_NONE );
    update_pos( victim );
