...
int	angry_gain	args( ( CHAR_DATA *ch ) );
...
void advance_level( CHAR_DATA *ch )
{
    ...
    add_ap	= class_table[ch->class].fAngry_point;

    add_hp               = UMAX(  1, add_hp   );
    add_mana             = UMAX(  0, add_mana );
    add_move             = UMAX( 10, add_move );
    add_ap		 = UMAX(  1, add_ap   );
    ...
    if ( !IS_NPC( ch ) )
	    ch->pcdata->max_ap	+= add_ap;

    if ( !IS_NPC( ch ) )
	REMOVE_BIT( ch->act, PLR_BOUGHT_PET );

    sprintf( buf,
	    "Your gain is: %d/%d hp, %d/%d m, %d/%d mv, %d/%d ap %d/%d prac.\n\r",
	    add_hp,	ch->max_hit,
	    add_mana,	ch->max_mana,
	    add_move,	ch->max_move,
	    IS_NPC( ch ) ? 100 : add_ap,
...
void demote_level( CHAR_DATA *ch )
{
    ...
    ch->level -= 1;
    sprintf( buf,
	    "Your loss is: %d/%d hp, %d/%d m, %d/%d mv, %d/%d ap %d/%d prac.\n\r",
	    add_hp,	ch->max_hit,
	    add_mana,	ch->max_mana,
	    add_move,	ch->max_move,
	    IS_NPC( ch ) ? 100 : add_ap,
...
int angry_gain( CHAR_DATA *ch )
{
    ...
    if ( ch->pcdata->angry_point - gain < 30 )
    	return ch->pcdata->angry_point - 1;
    else return gain;
}
...
{   
	...
	if ( ch->position >= POS_STUNNED )
	{
		...
		ch->move += move_gain( ch );
            ...
            if ( !IS_NPC( ch ) )
                if ( ch->pcdata->angry_point <= MAX_AP )
                    ch->pcdata->angry_point -= angry_gain( ch );
	}
...
void auction_update (void)
{
    ...
    if (auction->item != NULL)
        if (--auction->pulse <= 0) /* decrease pulse */
        {
            ...
            case 2 : /* going twice */
            if (auction->bet > 0)
                sprintf (buf, "[1;35m%s[1;35m¥s½æ%s¥s%s¤w¸g¥s¨ì[1;33m%d[1;35m¤¸¤F¡C[m",auction->seller->cname, auction->item->cname,
                     ((auction->going == 1) ? "²Ä¤@¦¸" : "²Ä¤G¦¸"), auction->bet);
            else
                sprintf (buf, "[1;35m%s[1;35m¥s½æ%s%s, ÁÙ¨S¦³¤H¥X»ù¡A­n¥s§Ö¥s¡C[m",auction->seller->cname, auction->item->cname,
                     ((auction->going == 1) ? "²Ä¤@¦¸" : "²Ä¤G¦¸"));
            ...
            if (auction->bet > 0)
            {
                sprintf (buf, "[1;35m%s[1;35m¥H[1;33m %d [1;35m¤¸½æµ¹%s[1;35m¡C[m",
                    auction->item->cname,
                    auction->bet,
                    auction->buyer->cname );
                talk_auction(buf);
                obj_to_char (auction->item,auction->buyer);
                act ("[1;34m¶Â¥«¥æ©ö­û¬ðµM¥X²{¦b§A­±«e§â$h¥æµ¹§A«á´N®ø¥¢¤F¡C[m",
                     auction->buyer,auction->item,NULL,TO_CHAR);
                act ("[1;34m¶Â¥«¥æ©ö­û¬ðµM¥X²{¦b$c[1;34m¡A§â$h¥æµ¹$m[1;34m¡AµM«á´N®ø¥¢¤F¡C[m",
            ...
            {
                sprintf (buf, "[1;35m¨S¦³¤H²z·|§Aªº%s¡C[m",auction->item->cname );
                talk_auction(buf);
                act ("[1;34m¶Â¥«¥æ©ö­û¥X²{¦b§A­±«e§â$hÁÙµ¹§A¡C[m",
                      auction->seller,auction->item,NULL,TO_CHAR);
                act ("[1;34m¶Â¥«¥æ©ö­û¥X²{¦b$c­±«e§â$h¥æµ¹$m[1;34m¡C[m",
