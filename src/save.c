 ...
 * Just for safe if the player's file is corrupted.
 */
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	{
	    if ( skill_table[sn].name && ch->pcdata->learned[sn] > 0 && ( str_cmp( skill_table[sn].name, "(null)" ) ) )
	    {
		fprintf( fp, "Skll        %d %d '%s'\n",
		    ch->pcdata->learned[sn], ch->skill_level[sn], skill_table[sn].name );
	    }
	}
    }
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->pcdata->perm_str		= 17;
    ch->pcdata->perm_int		= 17; 
    ch->pcdata->perm_wis		= 17;
    ch->pcdata->perm_dex		= 17;
    ch->pcdata->perm_con		= 17;
    ...
    ch->pcdata->learned		= alloc_perm( sizeof( int ) * MaxSkill );
	...
	for ( ; ; )
	{
		...
		write_to_buffer( d, "[1;35m±zªºÀÉ®×¬Ý¨Ó¬O¤w¸gÃa±¼¤F, ¨t²Î¹Á¸Õ¬°±z¥s¥X³Æ¥÷ÀÉ¡C[m\n\r", 0 );
		load_char_obj_backup( d, name );
		break;
	    }
...
bool load_char_obj_backup( DESCRIPTOR_DATA *d, char *name )
{
    extern char      *daPrompt;
           FILE      *fp;
	   CHAR_DATA *ch;
    ...
    /* decompress if .gz file exists - Thx Alander */
#if !defined( macintosh ) && !defined( WIN32 )
    sprintf( strsave, "%s%s%s%s%s", BACKUP_DIR, initial( ch->name ),
	    "/", capitalize( name ), ".gz" );
    if ( ( fp = fopen( strsave, "r" ) ) )
    {
        char       buf     [ MAX_STRING_LENGTH ];
	...
	for ( ; ; )
	{
	    ...
	    if ( letter != '#' )
	    {
	        sprintf( buf, "Load_char_obj_backup: Cannot find the backup of %s.", name );
		bug( buf, 0 );
		write_to_buffer( d, "[1;33m±zªº³Æ¥÷ÀÉ®×¬Ý¨Ó¬O¤w¸gÃa±¼¤F¡C\n\r³Â·Ð½Ð§A³qª¾¤j¯«¨Ó³B²z, ÁÂÁÂ¡C[m\n\r", 0 );
		break;
	    }
	    else if ( letter == '#' ){
  		write_to_buffer( d, "[1;37m§ä¨ì¤F±zªº³Æ¥÷¨ÃÅª¨ú§¹²¦¡C[m\n\r", 0 );	    
	    
	    sprintf( buf, "Load_char_obj_backup: Load the backup of %s.", name );
	    bug( buf, 0 );
		...
		{
		    sprintf( buf,
			    "Load_char_obj_backup:  %s section PLAYER corrupt.\n\r",
			    name );
		    bug( buf, 0 );
		    write_to_buffer( d, sorry_player, 0 );
		...
		{
		    sprintf( buf,
			    "Load_char_obj_backup:  %s section OBJECT corrupt.\n\r",
			    name );
		    bug( buf, 0 );
		    write_to_buffer( d, sorry_object, 0 );
		    return FALSE;
	    ...
	    {
		bug( "Load_char_obj_backup: bad section.", 0 );
		break;
	    }
	} /* for */
    ...
    {
        write_to_buffer( d, "[1;34m§Aªº³Æ¥÷ÀÉ§ä¤£¨ì¡C«Ü©êºp, ½Ð±z­«½m¤@°¦¤F¡C[m\n\r", 0 );
    }    
...
int fread_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    char        def_prompt [] = "<¥Í[0;35m%h[mºë[1;34m%m[mÅé[0;32m%v[m°«[0;33m%A[m> ";
    ...
    char	def_cname  [] = "Your Chinese Name was corrupted.";
    ...
    struct key_data key_tab [] = {
      ...
      { "LastFrom", TRUE, DEFLT,		{ &ch->pcdata->lastFrom, NULL }},
      { "LastName", TRUE, DEFLT,		{ &ch->pcdata->lastName, NULL }},
...
int fread_obj( CHAR_DATA *ch, FILE *fp )
{
	...
	{ "CName",       TRUE,  (int) &corobj,	  { &obj.cname,	      NULL } },
        ...
        else if ( !str_cmp( word, "Vnum" ) )
	{
		...
		log_string( "Get something wrong in fread_obj." );
 	    }	
