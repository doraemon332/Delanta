...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    buf[0] = '\0';
    if ( IS_OBJ_STAT( obj, ITEM_INVIS)     )   strcat( buf, "([0;30mÁô§Î[m) "     );
    if ( ( IS_AFFECTED( ch, AFF_DETECT_EVIL  )
	  || ( IS_SET( race_table[ ch->race ].race_abilities,
		      RACE_DETECT_ALIGN )
	      && IS_GOOD( ch ) ) )
	&& IS_OBJ_STAT( obj, ITEM_EVIL )   )   strcat( buf, "([1;31m¬õ¥ú[m) "  );
    if ( IS_AFFECTED( ch, AFF_DETECT_MAGIC )
	&& IS_OBJ_STAT( obj, ITEM_MAGIC )  )   strcat( buf, "([0;34mÅ]ªk[m) "   );
    if ( IS_OBJ_STAT( obj, ITEM_GLOW )     )   strcat( buf, "([1;31mµo«G[m) "   );
    if ( IS_OBJ_STAT( obj, ITEM_HUM )      )   strcat( buf, "([1;32m¶ä¶ä§@ÅT[m) "   );
    if ( IS_OBJ_STAT( obj, ITEM_POISONED ) )   strcat( buf, "([0;32m¦³¬r[m) "  );
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	&& get_trust( victim ) < get_trust( ch ) )
                                                strcat( buf, "([1;37m¯«¤§¹P[m) "   );
    if ( IS_AFFECTED( victim, AFF_INVISIBLE )   )
                                                strcat( buf, "([1;37mÁô¨­[m) "      );
    if ( IS_AFFECTED( victim, AFF_HIDE )        )
                                                strcat( buf, "([0;37m¸úÂÃ[m) "       );
    if ( IS_AFFECTED( victim, AFF_CHARM )       )
                                                strcat( buf, "([1;37m°g¥¢¤ß´¼[m) "    );
	...
	|| ( IS_SET( race_table[ victim->race ].race_abilities, RACE_PASSDOOR )
		...
		|| !str_cmp( race_table[ victim->race ].name, "vampire" ) ) ) )
                                                strcat( buf, "([1;36m¥b³z©ú[m) ");
    if ( IS_AFFECTED( victim, AFF_FAERIE_FIRE ) )
                                                strcat( buf, "([1;31m¯»¬õ®ð¹Î[m) "  );
    if ( IS_EVIL( victim )
	&& ( IS_AFFECTED( ch, AFF_DETECT_EVIL )
	    || ( IS_SET( race_table[ ch->race ].race_abilities,
			RACE_DETECT_ALIGN )
		&& IS_GOOD( ch ) ) ) )
                                                strcat( buf, "([0;31m¬õ¦â¸t¥ú[m) "   );
    if ( IS_AFFECTED( victim, AFF_SANCTUARY )
	|| IS_SET( race_table[ victim->race ].race_abilities, RACE_SANCT ) )
                                                strcat( buf, "([1;33m¥Õ¦â¸t¥ú[m) " );
    if ( IS_AFFECTED( victim, AFF_FLAMING ) )   strcat( buf, "([0;31m·t¬õ¦â¨¸®ð[m) " );
    if ( !IS_NPC( victim ) && IS_SET( victim->act, PLR_KILLER )  )
						strcat( buf, "([1;32m±þ¤H¥Ç[m) "     );
    if ( !IS_NPC( victim ) && IS_SET( victim->act, PLR_THIEF  )  )
						strcat( buf, "([1;32mÅÑ¸é[m) "      );
    ...
    {
    case POS_DEAD:     strcat( buf, "¦º¦b³o¸Ì!!"                 ); break;
    case POS_MORTAL:   strcat( buf, "¦b³o¸ÌÅuµÛ, ¦n¹³ÁÙ¦³¥Í©R¡C" ); break;
    case POS_INCAP:    strcat( buf, "¦b³o¸ÌÅuµÛ¡C"               ); break;
    case POS_STUNNED:  strcat( buf, "¨ü¤F­«¶Ë½ö¦b³o¸Ì¡C"	 ); break;
    case POS_SLEEPING: strcat( buf, "¦b³o¸ÌºÎÄ±¡C"	         ); break;
    case POS_RESTING:  strcat( buf, "§¤¦b³o¸Ì¥ð®§¡C"             ); break;
    case POS_STANDING: strcat( buf, "¯¸¦b³o¸Ìµo§b¡C"             ); break;
    case POS_FIGHTING:
	strcat( buf, "¥¿¦b³o¸Ì©M" );
	if ( !victim->fighting )
	    strcat( buf, "ªÅ®ð" );
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	    ...
	    && can_see_obj( ch, obj ) )
	{
	    ...
	    {
		send_to_char( "\n\r", ch );
		act( "$E¥¿¨Ï¥ÎµÛ", ch, NULL, victim, TO_CHAR );
		found = TRUE;
	    }
	...
	if ( !IS_NPC( ch ) && IS_SET( ch->act, PLR_LOOKSKY ) && IS_OUTSIDE( ch ) )
	{
	    ...
	    switch ( weather_info.sky )
	    {
	        default:
	        	...
	        	strcat( buf, "¡@¡@¤ÑªÅ¶}©l¦³¤@ÂI¶³Ãú" );
        		break;
        	case SKY_CLOUDLESS:
        		strcat( buf, "¡@¡@¤ÑªÅ¶}©l¦³¤@ÂI¶³Ãú" );
        		break;
        	case SKY_CLOUDY:
        		strcat( buf, "¡@¡@¤ÑªÅ¤¤¦³¤p«Bºw¸¨¤U" );
        		break;
        	case SKY_RAINING:
        		strcat( buf, "¡@¡@¤Ñ¤W¥¿¤UµÛ¤j«B" );
        		break;
        	case SKY_LIGHTNING:
        		strcat( buf, "¡@¡@°{¹q±q¤ÑªÅ¤¤¸¨¤U" );
        		break;
            }
...
void do_exits( CHAR_DATA *ch, char *argument )
{
    ...
    strcpy( buf, fAuto ? "    [0;37m©úÅãªº¥X¤f¦³[1;37m " : "³o¸Ìªº¥X¤f¡G\n\r" );
    ...
    if ( !found )
	strcat( buf, fAuto ? "¨S¦³¥ô¦ó¥X¤f" : "¨S¦³¡C\n\r" );
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf + strlen( buf ), "                         [1;36mÁ¼ÇU»â°ì¡Ð¼wÄõ¶ð[m\n\r" );
    sprintf( buf + strlen( buf ), "*------------------------------------------------------------------------------*\n\r" );
    sprintf( buf + strlen( buf ), "¥Ø«e¦³[1;33m%3d[m¦ìª±®a¦b½u¤W, ¦³[1;32m%3d[m¦ì¬O¤j¯«, ¦³[1;31m%3d[m¦ì¬O¹ê²ß¯«\n\r", players_count, directs_count, immortals_count );
    sprintf( buf + strlen( buf ), "*------------------------------------------------------------------------------*\n\r" );        
    sprintf( buf + strlen( buf ), "[1;33m[µ¥¯Å  Â¾  ·~   Â¾  ºÙ ][          ±Ô                       ­z                 ][0;37;40m\n\r" );
    sprintf( buf + strlen( buf ), "*------------------------------------------------------------------------------*\n\r" );         
	...
	if ( wch->level >= LEVEL_HERO )
	    switch ( wch->level )
	      {
	      default:    break;
	      case L_DIR: advance_class = "ºÞ²z¤Ñ¯«"; break;
	      case L_SEN: advance_class = "°Ï°ìÇU¯«"; break;
	      case L_JUN: advance_class = "°õªk¤Ñ¯«"; break;
	      case L_APP: advance_class = "¹ê²ßÇU¯«"; break;
	      case LEVEL_HERO: advance_class = "¸tªÌ"; break;
	      }
	strcat( buf3, wch->cname );
	strcat( buf3, " " );
	strcat( buf3, wch->pcdata->title );
	strcat( buf3, IS_SET( wch->act, PLR_WIZINVIS ) ? "(¤j¯«Áô¨­) " : "" );
	strcat( buf3, IS_SET( wch->act, PLR_KILLER   ) ? "(±þ¤H¥Ç) " : "" );
	strcat( buf3, IS_SET( wch->act, PLR_THIEF    ) ? "(¤p°½) "  : "" );
	strcat( buf3, IS_SET( wch->act, PLR_REGISTER ) ? "(±µ¨üPK) "     : "" );
	strcat( buf3, IS_SET( wch->act, PLR_AFK      ) ? "(¼È®ÉÂ÷¶}Áä½L) "    : "" );
...
void do_cname( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        send_to_char( "§A·Q­n§â¤¤¤å¦W¦r§ï¦¨¤°»ò©O??\n\r", ch );
        return;
    }
    ...
    sprintf( buf, "§Aªº¤¤¤å¦W¦r¤w¸g§ï¦¨¤F %s .\n\r", ch->cname );
    send_to_char( buf, ch );
}
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(  IS_SET( ch->act, PLR_AUTOTIME  )
	    ? "[+AUTOTIME ] §A·|¦Û°Ê¬Ý®É¶¡¡C\n\r"
	    : "[-autotime ] §A¤£¦Û°Ê¬Ý®É¶¡¡C\n\r"
	    , ch );
	    
	send_to_char(  IS_SET( ch->act, PLR_LOOKSKY   )
	    ? "[+LOOKSKY  ] §A·|¬Ý¬Ý¤ÑªÅªº±¡ªp¡C\n\r"
	    : "[-looksky  ] §A¤£·|¥h²z¤ÑªÅªº±¡ªp¡C\n\r"
	    , ch );
...
void do_slist ( CHAR_DATA *ch, char *argument )
{
    ...
    strcat ( buf1, "¦s¦b¦b³o­Ó¥@¬É, §AªºÂ¾·~¥i¥H¾Ç²ß¨ìªº§Þ¯à¡C\n\r\n\r" );
    strcat ( buf1, "§Þ¯à¡G\n\r\n\r" );
...
void do_prompt( CHAR_DATA *ch, char *argument )
{
   ...
   if( !strcmp( argument, "all" ) )
      strcat( buf, "<¥Í[0;35m%h[mºë[1;34m%m[mÅé[0;32m%v[m°«[0;33m%A[m> ");
   else
   {
      if ( strlen( argument ) > 50 )
	  argument[50] = '\0';
