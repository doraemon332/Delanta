...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
    ...
    {
	act( "§A¤£¯à²¾°£$h¡C", ch, obj, NULL, TO_CHAR );
	return FALSE;
    }

    unequip_char( ch, obj );
    act( "$c°±¤î¨Ï¥Î$h¡C", ch, obj, NULL, TO_ROOM );
    act( "§A°±¤î¨Ï¥Î$h¡C", ch, obj, NULL, TO_CHAR );
    return TRUE;
}
...
/*
 * Unequip all objects on a character.
 */
void unequip_all_obj( CHAR_DATA *ch )
{
    OBJ_DATA *obj;
    int iWear;
...
/*
 * Remove all objects on a character.
 */
...
void do_poison_weapon( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch )
	|| (  !IS_NPC( ch )
	    && !IS_LEARNED( ch, gsn_poison_weapon ) ) )
    {                                          
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if (arg1[0] == '\0')
        if (auction->item != NULL)
        {
            /* show item data here */
            if (auction->bet > 0)
                sprintf (buf, "[1;35m²{¦b¤w¸g¥s»ù¨ì[1;33m %d [1;35m¤¸¤F¡C\n\r",auction->bet);
            else
                sprintf (buf, "[1;35m³o­ÓªF¦è²{¦bÁÙ¨S¦³¤H¥s»ù¡C\n\r");
            send_to_char (buf,ch);
    ...
    if (auction->item == NULL)
    {
        send_to_char ("¨S¦³ªF¦è¥¿¦b©ç½æ, §A°±¤£¤F©ç½æ¡C\n\r",ch);
        return;
    }
    else /* stop the auction */
    {
        sprintf (buf,"[1;35m%sªº©ç½æ³Q¤Ñ¯«ªý¤î¤F, ªF¦è­¸¦^¨Ó§Aªº¨­¤W¡C[m",
                        auction->item->cname );
        talk_auction (buf);
        ...
        if (auction->buyer != NULL) /* return money to the buyer */
        {
            ...
            send_to_char ("[1;35m§Aªº¿ú¤]³Q°h¦^¨Ó¤F¡C[m\n\r",auction->buyer);
        }
        return;
    }
    ...
    if (!str_cmp(arg1,"bet") )
	    ...
	    /* to avoid slow auction, use a bigger amount than 100 if the bet
            ...
            */
            ...
            {
                send_to_char ("§A¦Ü¤Ö­n¥X 100 ¶ô¿ú!!\n\r",ch);
                return;
            }

            if (newbet > ch->gold)
            {
                send_to_char ("§A¨S¦³¨º»ò¦hªº¿ú¡I\n\r",ch);
                return;
            }
            ...
            sprintf (buf,"[1;35m%s[1;35m¥s»ù¡G¡u§Ú­n¥X [1;33m%d [1;35m¤¸¶R%s¡C¡v[m\n\r",auction->buyer->cname, newbet,auction->item->cname );
            talk_auction (buf);
            return;
...
/* finally... */
    ...
    default:
        act ("§A¤£¯à©ç½æ$T¡C",ch, NULL, item_type_name (obj), TO_CHAR);
        return;
...
*/
    ...
    case ITEM_SCROLL:
        ...
        sprintf (buf, "[1;35m%s[1;35m¥s½æ¡G¡u½Ö­n¥X¿ú¶R§Úªº%s°Ú¡H¡v[m",auction->seller->cname,  obj->cname );
        talk_auction (buf);
    ...
    {
        act ("½Ðµy«á¦A¹Á¸Õ¡C²{¦b¦³¤H¦b©ç½æ$h!!",ch,auction->item,NULL,TO_CHAR);
        return;
    }
}