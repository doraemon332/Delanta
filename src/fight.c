...
void damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn )
{
    ...
    /*
     * Let the angry point to damage others more or less .
     */
    if ( !IS_NPC( ch ) )
        dam += dam * ch->pcdata->angry_point / ch->pcdata->max_ap;
    if ( !IS_NPC( victim ) )
    {
        int addon;
        
        addon = dam * 100 / ( victim->level );
        victim->pcdata->angry_point = UMIN( victim->pcdata->max_ap, victim->pcdata->angry_point + addon );
    }        
	...
	if ( IS_NPC( ch ) && ch->desc )
	    sprintf( buf,
		    "Damage: %d from %s by %s: > 10000000 points with %d dt!",
		    dam, ch->name, ch->desc->original->name, dt );
	else
	    sprintf( buf,
		    "Damage: %d from %s: > 10000000 points with %d dt!",
		    dam, ch->name, dt );
    ...
    if ( dt != TYPE_UNDEFINED )
    {
        if ( !IS_NPC( ch ) )
        {
            ...
            delete = dam * ( ch->level ) / 100;
            ch->pcdata->angry_point = UMAX( MIN_AP, ch->pcdata->angry_point - delete );
            if ( dt >= 0 && dt <= MaxSkill )
            {
                dam *= ch->skill_level[dt] / 1000;
                ch->skill_level[dt] = UMIN( MAX_SKILL_EXP, ch->skill_level[dt] + ( dam / 100 ) );
            }
        }
    ...
    case POS_MORTAL:
	send_to_char( 
	    "[0;31m§A¨ü¤F­«¶Ë, ¦pªG¤£»°§ÖªvÀø, ·|¦³¦º¤`ªº¥i¯à¡C[0m\n\r",
	    victim );
    ...
    case POS_DEAD:
	send_to_char( "[0;31m§A³Q±þ±¼¤F!![0;37;40m\n\r\n\r", victim );
	act( "[0;31m$c[0;31m¦º¤F!![0;37;40m", victim, NULL, NULL, TO_ROOM );
	break;
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( "$C¾×¦í¤F§Aªº§ðÀ»!!", ch, NULL, victim, TO_CHAR );
    act( "§A¾×¦í¤F$cªº§ðÀ»!!",  ch, NULL, victim, TO_VICT );
    return TRUE;
}
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( "$CÁa¨­¤@¸õ¸ú¹L¤F§Aªº§ðÀ»!!", ch, NULL, victim, TO_CHAR    );
    act( "§A¸ú¹L¤F$cªº§ðÀ»!!", ch, NULL, victim, TO_VICT    );
    return TRUE;
}
		 ...
		 int wpn )
{
	...
	if ( victim->hit > 0 )
	    dam /= victim->hit;
             ...
             if ( dam <=   1 ) { vp = "»´·Lªº¶Ë®`";           }
	else if ( dam <=   2 ) { vp = "»´·L¶Ë®`";              }
	else if ( dam <=   4 ) { vp = "[0;32m»´¶Ë[m";                }
	else if ( dam <=   6 ) { vp = "[0;32m¶Ë®`[m";             }
	else if ( dam <=   8 ) { vp = "[0;32m·ï«C¶Ë®`[m";              }
	else if ( dam <=  10 ) { vp = "[1;31mÄY­«ªº¶Ë®`[m";               }
	else if ( dam <=  20 ) { vp = "[1;31mÄY­«ªº¶Ë®`[m";           }
	else if ( dam <=  30 ) { vp = "[1;31m«ÜÄY­«ªº¶Ë®`[m";          }
	else if ( dam <=  40 ) { vp = "[0;31m§Ö­n´Ý¼o¤F[m";               }
	else if ( dam <=  50 ) { vp = "[0;31m¤w¸g³£´Ý¼o¤F[m";           }
	else if ( dam <=  60 ) { vp = "[0;31m¤ºÅ¦³£§Ö¶]¥X¨Ó¤F[m";         }
	else if ( dam <=  70 ) { vp = "[0;31m¤ºÅ¦³£¤w¸g¶]¥X¨Ó¤F[m";         }
	else if ( dam <=  80 ) { vp = "[0;31m±µªñ¦º¤`Ãä½t[m";           }
	else if ( dam <=  90 ) { vp = "[0;31m§¹¥þ­P©Rªº¶Ë®`[m";  }
	else                   { vp = "[0;31m·´·À©Êªº¶Ë®`[m"; }
    }
        ...
        if ( ch->race > MAX_RACE )
	{
        ...
        if ( dama < 9999 )
        {
	    sprintf( buf1, "§Aªº%s¨Ï$C³y¦¨%s%c(%d)",      attack, vp, punct, dama );
	    sprintf( buf2, "$cªº%s¨Ï§A³y¦¨%s%c(%d)",       attack, vp, punct, dama );
	    sprintf( buf3, "$cªº%s¨Ï$C³y¦¨%s%c(%d)",        attack, vp, punct, dama );
	    sprintf( buf4, "§Aªº%s¨Ï§A¦Û¤v³y¦¨%s%c(%d)", attack, vp, punct, dama );
	    sprintf( buf5, "$cªº%s¨Ï$m³y¦¨%s%c(%d)", attack, vp, punct, dama );
	}
	else 
	{
	    sprintf( buf1, "§Aªº%s¨Ï$C³y¦¨%s%c(****)",      attack, vp, punct );
	    sprintf( buf2, "$cªº%s¨Ï§A³y¦¨%s%c(****)",       attack, vp, punct );
	    sprintf( buf3, "$cªº%s¨Ï$C³y¦¨%s%c(****)",        attack, vp, punct );
	    sprintf( buf4, "§Aªº%s¨Ï§A¦Û¤v³y¦¨%s%c(****)", attack, vp, punct );
	    sprintf( buf5, "$cªº%s¨Ï$m³y¦¨%s%c(****)", attack, vp, punct );	
	}
    }
    else
    {
	...
	else
	{
        ...
        if ( ( obj = get_eq_char( ch, wpn ) ) != NULL && ( wpn != WEAR_NONE ) )
	        strcpy( weapon, obj->cname );
	else if ( wpn == WEAR_NONE )
	{
	     strcpy( weapon, "\b\b\b\b¨Ï¥X" );
	     strcpy( degree, "" );
	}
	else
	     strcpy( weapon, "\b\b\b\b" );
	...
	if ( dt > TYPE_HIT && is_wielding_poisoned( ch, wpn ) )
	{
	    if ( dama < 9999 )
	    {
	    	sprintf( buf1, "§A¤â«ù%s%sªº%s³y¦¨$c¤¤¬r¥B%s%c(%d)",  weapon, degree, attack, vp, punct, dama );
		sprintf( buf2, "$c¤â«ù%s%sªº%s³y¦¨§A¤¤¬r¥B%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf3, "$c¤â«ù%s%sªº%s³y¦¨$c¤¤¬r¥B%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf4, "§A¤â«ù%s%sªº%s³y¦¨§A¦Û¤v¤¤¬r¥B%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf5, "$c¤â«ù%s%sªº%s³y¦¨$m¤¤¬r¥B%s%c(%d)", weapon, degree, attack, vp, punct, dama );	    
	    }
	    else 
	    {
	    	sprintf( buf1, "§A¤â«ù%s%sªº%s³y¦¨$C¤¤¬r¥B%s%c(****)",  weapon, degree, attack, vp, punct );
		sprintf( buf2, "$c¤â«ù%s%sªº%s³y¦¨§A¤¤¬r¥B%s%c(****)", weapon, degree, attack, vp, punct );
	        sprintf( buf3, "$c¤â«ù%s%sªº%s³y¦¨$C¤¤¬r¥B%s%c(****)", weapon, degree, attack, vp, punct );
	        sprintf( buf4, "§A¤â«ù%s%sªº%s³y¦¨§A¦Û¤v¤¤¬r¥B%s%c(****)", weapon, degree, attack, vp, punct );
	        sprintf( buf5, "$c¤â«ù%s%sªº%s³y¦¨$m¤¤¬r¥B%s%c(****)", weapon, degree, attack, vp, punct );	    	    
	    }
	}
	else
	{
	    if ( dama < 9999 )
	    {
	        sprintf( buf1, "§A¤â«ù%s%sªº%s³y¦¨$C%s%c(%d)",  weapon, degree, attack, vp, punct, dama );
	        sprintf( buf2, "$c¤â«ù%s%sªº%s³y¦¨§A%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf3, "$c¤â«ù%s%sªº%s³y¦¨$C%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf4, "§A¤â«ù%s%sªº%s³y¦¨§A¦Û¤v%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	        sprintf( buf5, "$c¤â«ù%s%sªº%s³y¦¨$m%s%c(%d)", weapon, degree, attack, vp, punct, dama );
	    }
	    else 
	    {
	        sprintf( buf1, "§A¤â«ù%s%sªº%s³y¦¨$C%s%c(****)",  weapon, degree, attack, vp, punct );
	        sprintf( buf2, "$c¤â«ù%s%sªº%s³y¦¨§A%s%c(****)", weapon, degree, attack, vp, punct  );
	        sprintf( buf3, "$c¤â«ù%s%sªº%s³y¦¨$C%s%c(****)", weapon, degree, attack, vp, punct );
	        sprintf( buf4, "§A¤â«ù%s%sªº%s³y¦¨§A¦Û¤v%s%c(****)", weapon, degree, attack, vp, punct );
	        sprintf( buf5, "$c¤â«ù%s%sªº%s³y¦¨$m%s%c(****)", weapon, degree, attack, vp, punct );	    
	    }
	}
    }
